...
const { Option } = Select;
const stepItems = [
  { key: 'basic', title: '基础信息', icon: <TeamOutlined /> },
  { key: 'case', title: '选择用例', icon: <ApiOutlined /> },
  { key: 'notify', title: '通知设置', icon: <NotificationOutlined /> },
];
...
const CaseList: React.FC<CaseListProps> = ({
  ...
  useEffect(() => {
    onSave({
      selectedCaseData: (form.getFieldValue('case_list') || []).map(
    ...
    <>
      <Col span={24}>
        <Form.Item
          label="用例树"
          name="case_list"
          rules={[{ required: true, message: '请至少选择一个case' }]}
          {...CONFIG.SQL_LAYOUT}
        >
          <TreeSelect
            treeData={treeData}
            ...
            onChange={(a: any, b: any) => {
              onSave({
                selectedCaseData: b.map((item: string, idx: number) => ({
                  name: item,
                  case_id: a[idx],
                  index: idx,
                })),
      ...
      <Col span={24} style={{ marginTop: 8 }}>
        <Form.Item label="用例表" name="caseList" {...CONFIG.SQL_LAYOUT}>
          <SortedTable
            ...
            dragCallback={(data) => {
              form.setFieldsValue({ case_list: data.map((item: any) => item.case_id) });
            }}
          />
        </Form.Item>
      </Col>
...
const TestPlanForm: React.FC<TestPlanFormProps> = ({
  user,
  loading,
  project,
  testplan,
    ...
    if (planRecord.id) {
      res = await dispatch({
        ...
        payload: {
          ...values,
          id: planRecord.id,
          case_list: values.case_list.map((item: string) => parseInt(item.split('_')[1], 10)),
        },
      });
    } else {
      res = await dispatch({
      ...
      return (
        <>
          ...
          <Col span={12}>
            <Form.Item
              label="计划名称"
              rules={[{ required: true, message: '请填写测试计划名称' }]}
              name="name"
            >
          ...
          <Col span={12}>
            <Form.Item
              label="运行环境"
              rules={[{ required: true, message: '请选择要运行的环境, 支持多选' }]}
              name="env"
            >
          ...
          <Col span={12}>
            <Form.Item
              label="优先级"
              rules={[{ required: true, message: '请选择测试计划优先级' }]}
              name="priority"
            >
              <Select placeholder="选择优先级" allowClear>
                {CONFIG.PRIORITY.map((v: any) => (
          ...
          <Col span={12}>
            <Form.Item
              label="cron表达式"
              extra={
                <div className="m-input-footer-msg">{cronDate || '* cron表达式只支持5位!'}</div>
              }
              rules={[
                { required: true },
                {
                  validator: (_, value) => {
                    ...
                    try {
                      ...
                      setCronDate(
                        `下次运行时间: ${moment(new Date(date.next().toString())).format(
                          'YYYY-MM-DD HH:mm:ss',
                        )}`,
                      );
          ...
          <Col span={12}>
            <Form.Item
              label="顺序执行"
              rules={[{ required: true, message: '请选择测试计划是否需要顺序执行' }]}
              name="ordered"
            >
      ...
      return (
        <>
          <Col span={12}>
            <Form.Item
              label="合格率(%)"
              rules={[{ required: true, message: '请输入测试计划的最低通过率' }]}
              name="pass_rate"
            >
              <InputNumber
                placeholder="请输入测试计划的最低通过率"
                style={{ width: '100%' }}
                max={100}
                min={1}
              />
          ...
          <Col span={12}>
            <Form.Item label="重试等待(min)" rules={[{ required: false }]} name="retry_minutes">
              <InputNumber
                placeholder="重试等待时间, 不填则不重试"
                style={{ width: '100%' }}
                min={0}
              />
            </Form.Item>
          ...
          <Col span={24}>
            <Form.Item
              label="推送方式"
              rules={[{ required: false, message: '请选择推送方式' }]}
              name="msg_type"
              {...CONFIG.SQL_LAYOUT}
            >
              <Select allowClear showSearch placeholder="请选择推送方式" mode="multiple">
                {Object.keys(CONFIG.MSG_TYPE).map((key: any) => (
                  <Option key={key} value={key}>
                    <IconFont
                      type={CONFIG.MSG_ICON[key as keyof typeof CONFIG.MSG_ICON]}
                      style={{ fontSize: 18 }}
                    />{' '}
                    {CONFIG.MSG_TYPE[key as keyof typeof CONFIG.MSG_TYPE]}
                  </Option>
                ))}
              </Select>
            </Form.Item>
          ...
          <Col span={24}>
            <Form.Item
              label="推送用户"
              rules={[{ required: false, message: '请选择推送人员' }]}
              name="receiver"
              {...CONFIG.SQL_LAYOUT}
            >
        ...
        <Row gutter={[8, 8]} style={{ marginTop: 24 }}>
          ...
          <div style={{ margin: '24px auto' }}>
            ...
            {currentStep < 2 ? (
              ...
              >
                <IconFont type="icon--xiayibu" /> 下一步
              </Button>
            ) : (
...
export default connect(({ user, loading, project, testplan, gconfig }: any) => ({
  user,
  loading,
  project,
  testplan,

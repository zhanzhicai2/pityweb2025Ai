...
import type { RequestInfo } from '@/types/RequestInfo.d';
...
type DataIndex = keyof RequestInfo;
...
const MethodTag = ({ color, text, fontColor }: TagProps) => {
  return (
...
const Detail = ({ name, record }: { name: string; record: RequestInfo }) => {
  return (
    <a
      onClick={() => {
        Modal.info({
...
const RequestInfoList: React.FC<RequestInfoProps> = ({
  ...
  const columns: ColumnsType<RequestInfo> = [
    {
      title: '编号',
      key: 'index',
      render: (text, record) => `${(record.index ?? 0) + 1}`,
    },
    {
      title: '请求地址',
      key: 'url',
      ...
      ...getColumnSearchProps('url' as DataIndex),
    },
    {
      title: '请求方式',
      key: 'request_method',
    ...
    },
    {
      ...
      title: '操作',
      render: (_, record) => (
        <Tooltip title="点击可删除录制数据(不推荐)">
          <DeleteTwoTone
            twoToneColor="#F56C6C"
            onClick={() => {
              onRemoveRecord(record.index ?? 0);
            }}
          />
        </Tooltip>
      ),
  ...
  return (
    <Table
      ...
      rowKey={(record) => (record as any)[(restProps.rowKey ?? 'index') as keyof RequestInfo]}
      loading={loading}

...
export default ({ loading, treeData, fetchData, projectData, userMap }) => {
  ...
  const Icon = (icon, title, operation, margin) => {
    return (
      <Tooltip title={title}>
        <span onClick={operation} style={{ marginLeft: margin }}>
          {icon}
        </span>
      </Tooltip>
    );
      ...
      <Alert
        ...
        description={
          <div>
            ...
            <p>
              {
                '3. 用例的生命周期是[变量替换]->[前置步骤执行]->[用例执行]->[后置步骤执行]->[断言执行]'
              }
              。
            </p>
  ...
  return (
    <Spin spinning={loading} tip="暴力加载中...">
      ...
      <ConstructorModal
        ...
        setModal={setConstructorModal}
        caseId={constructorCaseId}
        fetchData={fetchData}
      />
      <FormForModal
        open={assertModal}

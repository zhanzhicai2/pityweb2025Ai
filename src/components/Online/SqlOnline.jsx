...
const SqlOnline = ({
  ...
  const parseNewTree = (databaseSource) => {
    Object.keys(databaseSource).forEach((i) => {
      if (databaseSource[i].key.indexOf('column') > -1) {
        databaseSource[i].icon = databaseSource[i].primary_key ? (
    ...
    currentDatabase === null ? null : (
      <>
        <div>
          ...
          >
            {CONFIG.EDITOR_THEME.map((v, index) => (

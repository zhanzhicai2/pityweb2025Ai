...
const MiniProgress = ({ rate }: MiniProgressProps) => {
  const config = {
    height: 8,
    autoFit: false,
    percent: rate,
...
const Area = ({ data, field, color, fill }: AreaProps) => {
  const config = {
    height: 48,
    autoFit: true,
    line: {
...
export default () => {
  ...
  const getBonus = (item: { rank: number; count: number }) => {
    if (item.rank === 1) {
      return (
        <span className="rank-item">
          <IconFont type="icon-jin" /> <span className="rank-content-top">{item.count}</span>
        </span>
      );
    }
    if (item.rank === 2) {
      return (
        <span className="rank-item">
          <IconFont type="icon-yinpai" /> <span className="rank-content-top">{item.count}</span>
        </span>
      );
    }
    if (item.rank === 3) {
      return (
        <span className="rank-item">
          <IconFont type="icon-tongpai" /> <span className="rank-content-top">{item.count}</span>
        </span>
      );
    }
        ...
        <Col span={16}>
          <Row gutter={16}>
            <Col span={12}>
              <Skeleton loading={loading} active>
                <StatisticCard
                  className="statistics-card-large"
                  title="近一周用例通过率(%)"
                  statistic={{
                    value: report?.rate,
                    description: <MiniProgress rate={report?.rate} />,
                  }}
                  chart={<Area data={reportData} field="rate" />}
                />
              </Skeleton>
            </Col>
            <Col span={12}>
              <Skeleton loading={loading} active>
                <StatisticCard
                  className="statistics-card-large"
                  title={<span>近一周运行用例个数 </span>}
                  statistic={{
                    value: report?.count,
                  }}
                  chart={<Area data={reportData} field="count" />}
                />
              </Skeleton>
            </Col>
          </Row>
            ...
            <Col span={12}>
              <Skeleton loading={loading} active>
                <StatisticCard
                  ...
                  title={
                    <span>
                      {<Success theme="outline" size="15" fill="#7ed321" strokeLinecap="square" />}{' '}
                      近一周用例成功个数{' '}
                    </span>
                  }
                  statistic={{
            ...
            <Col span={12}>
              <Skeleton loading={loading} active>
                <StatisticCard
                  ...
                  title={
                    <span>
                      {<Jump theme="outline" size="15" fill="#4a90e2" strokeLinecap="square" />}{' '}
                      近一周用例跳过个数{' '}
                    </span>
                  }
                  statistic={{
          ...
          <Skeleton active loading={loading}>
            <Card title="用例数量排行Top10" className="rank">
              <List
                ...
                renderItem={(item: { id: number; rank: number; count: number }) => (
                      ...
                      <List.Item.Meta
                        ...
                        title={
                          <span>
                            ...
                            <Badge
                              style={{ marginLeft: 8 }}
                              status={clients[item.id] ? 'success' : 'default'}

...
const Notification = ({ user, dispatch }: NotificationProps) => {
  ...
  const { userMap } = user;
  const { fetchNotices, setNoticeCount, deleteNotices, readNotices, notices } = useModel(
    'notice',
  ) as any;
  ...
  const getNotices = async (currentTab: string) => {
...
export default connect(({ user }: { user: { userMap: UserMap } }) => ({ user }))(Notification);

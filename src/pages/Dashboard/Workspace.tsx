...
interface UserState {
  ...
  user_rank: number;
  total_user: number;
  weekly_case: { date: string; count: number }[];
  followPlan: FollowPlanItem[];
}
...
const getContent = (currentUser: CurrentUser) => {
  return (
    <div className={styles.pageHeaderContent}>
      ...
      <div className={styles.content}>
        <div className={styles.contentTitle}>{getWelcome(currentUser.name)}</div>
        <div>
...
const LinkTag = ({ name, link }: LinkTagProps) => {
  return (
    <Tag
      color="blue"
...
const Workspace = ({ user, dispatch }: WorkspaceProps) => {
  const { project_count, case_count, user_rank, total_user, weekly_case } = user;
  ...
  const onCalculateRate = (value: number) => {
    if (value < 0.1) {
      return 1;
    }
    if (value < 0.6) {
  ...
  const calculatePercent = (report: Report, pt = false) => {
    const percent = common.calPiePercent(
      report.success_count,
      report.success_count + report.failed_count + report.error_count,
    );
    if (pt) {
      return percent;
    }
  ...
  interface RingPieProps {
    report: Report;
  }

  const RingPie = ({ report }: RingPieProps) => {
    const config = {
      height: 128,
      autoFit: true,
  ...
  const revertArray = (report: Report[]) => {
    const temp = [...report];
    temp.reverse();
              ...
              {followPlan.length === 0 ? (
                <Col span={24}>
                  <Empty
                    ...
                    description={
                      <span>
                        你还没有关注测试计划, 赶紧去 <a href="/#/apiTest/testplan">关注</a> 一个吧！
                      </span>
                    }
                  />
                </Col>
              ) : (
                followPlan.map((item, idx) => (
                  <Col key={idx} span={24}>
                    <Card
                      size="small"
                      hoverable
                      ...
                      title={
                        <a href="/#/apiTest/testplan" style={{ fontSize: 16 }}>
                          {item.plan.name}
                        </a>
                      }
                    >
                            ...
                            >
                              <div style={{ textAlign: 'center' }}>
                                ...
                                <Rate
                                  disabled
                                  tooltips={desc}
                                  defaultValue={onCalculateRate(
                                    calculatePercent(item.report[0], true),
                                  )}
                                />
                                <span className="ant-rate-text">
                                  {
                                    desc[
                                      onCalculateRate(calculatePercent(item.report[0], true)) - 1
                                    ]
                                  }
                                </span>
                              </div>

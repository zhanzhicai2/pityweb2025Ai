...
const ReportDetail: React.FC<ReportDetailProps> = ({ dispatch, loading, user, gconfig }) => {
  ...
  const [currentCaseList, setCurrentCaseList] = useState<any[]>([]);
  const { envMap, envList } = gconfig;
  const { userMap } = user;

  const getTag = () => {
    if (
      reportDetail.failed_count === 0 &&
      reportDetail.error_count === 0 &&
      reportDetail.success_count > 0
    ) {
      return (
  ...
  const getPieData = () => {
    ...
    const total =
      ...
      reportDetail.error_count +
      reportDetail.skipped_count;
    return [
      {
        name: '成功',
        count: reportDetail.success_count,
        percent: common.calPiePercent(reportDetail.success_count, total),
      },
      {
        name: '失败',
        count: reportDetail.failed_count,
        percent: common.calPiePercent(reportDetail.failed_count, total),
      },
      {
        name: '错误',
        count: reportDetail.error_count,
        percent: common.calPiePercent(reportDetail.error_count, total),
      },
      {
        name: '跳过',
        count: reportDetail.skipped_count,
        percent: common.calPiePercent(reportDetail.skipped_count, total),
      },
    ];
  };

  const getReport = (record: any) => {
    return {
  ...
  const onSearchCase = (e: any) => {
    const { value } = e.target;
    const temp = caseList.filter(
      (item) => item.data_name.indexOf(value) > -1 || item.case_name.indexOf(value) > -1,
    );
    setCurrentCaseList(temp);
  };

  const load = !!(loading.effects['testcase/retryCase'] || loading.effects['gconfig/fetchEnvList']);
  ...
  const onHandleRetry = async (record: any) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: any) => (
        <a href={`/#/apiTest/testcase/${record.directory_id}/${record.case_id}`}>{text}</a>
      ),
    },
    {
      title: '数据描述',
    ...
    },
    {
      ...
      render: (status: number) => (
        <Badge
          status={reportConfig.EXECUTE_BADGE_STATUS[status]}
    ...
    },
    {
      ...
      key: 'method',
      render: (method: number) => reportConfig.METHOD_TAG[method],
    },
    {
      title: '开始时间',
      dataIndex: 'start_at',
          ...
          <Row gutter={[8, 8]}>
            <Col span={17}>
              ...
              <Descriptions>
                ...
                <Descriptions.Item label="测试结果">{getTag()}</Descriptions.Item>
                <Descriptions.Item label="执行人">
                  {reportDetail.executor === 0 ? (
                    'pity机器人'
                  ) : (
                    <UserLink user={userMap[reportDetail.executor]} size={16} />
                  )}
                </Descriptions.Item>
                ...
                <Descriptions.Item label="开始时间">{reportDetail.start_at}</Descriptions.Item>
                <Descriptions.Item label="结束时间">{reportDetail.finished_at}</Descriptions.Item>
                <Descriptions.Item label="耗时">
                  {parseFloat(reportDetail.cost) > 60
                    ? `${Math.round(parseFloat(reportDetail.cost) / 60)}分`
                    : reportDetail.cost + '秒'}
                </Descriptions.Item>
              </Descriptions>
            </Col>

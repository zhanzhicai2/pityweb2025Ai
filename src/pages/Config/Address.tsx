...
const Address: React.FC<AddressProps> = ({ loading, gconfig, dispatch }) => {
  ...
  const isLoading =
    loading.effects['gconfig/fetchAddress'] ||
    loading.effects['gconfig/fetchEnvList'] ||
    loading.effects['gconfig/updateAddress'] ||
    loading.effects['gconfig/insertAddress'] ||
    loading.effects['gconfig/deleteAddress'];
  ...
  const columns = [
    ...
    },
    {
      title: (
        <TooltipTextIcon
          title="地址一般是服务的基础地址，比如https://api.baidu.com, 用例中的地址简写即可"
          text="地址"
        />
      ),
      key: 'gateway',
  ...
  return (
    <PageContainer breadcrumb={undefined} title="请求地址管理">
      <Card>
        <FormForModal
          open={modal}
          fields={fields}
          title={item.id ? '修改地址' : '添加地址'}
          left={6}
          right={18}
          record={item}
          onFinish={onSubmit}
...
export default connect(({ gconfig, user, loading }: any) => ({ gconfig, user, loading }))(
  memo(Address),
);

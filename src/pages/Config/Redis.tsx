...
const Redis: React.FC<RedisProps> = ({ gconfig, loading, dispatch }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'cluster',
      dataIndex: 'cluster',
      render: (cluster: boolean) =>
        cluster ? <Tag color="blue">集群</Tag> : <Tag color="green">实例</Tag>,
    },
    {
      title: '操作',
      key: 'ops',
  ...
  return (
    <PageContainer title="Redis配置" breadcrumb={undefined}>
      <Card>
        <Form
          form={form}
          {...CONFIG.LAYOUT}
...
export default connect(({ gconfig, loading }: any) => ({ gconfig, loading }))(Redis);

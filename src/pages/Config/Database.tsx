...
const Database: React.FC<DatabaseProps> = ({ dispatch, gconfig, loading }) => {
  ...
  const columns = [
    {
      ...
      key: 'env',
      dataIndex: 'env',
      render: (text: number) => <Tag>{envMap[text]}</Tag>,
    },
    {
      title: '名称',
      key: 'name',
  ...
  const Footer = ({ onOk, onCancel, onTest }: any) => {
    return (
      <div>
  ...
  return (
    <PageContainer title="数据库配置列表" breadcrumb={undefined}>
      <Card>
        <FormForModal
          Footer={Footer}
          onTest={onTest}
...
export default connect(({ loading, gconfig }: any) => ({ loading, gconfig }))(Database);

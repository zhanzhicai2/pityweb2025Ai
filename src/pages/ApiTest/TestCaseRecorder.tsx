...
const TestCaseRecorder: React.FC<TestCaseRecorderProps> = ({
  dispatch,
  project,
  recorder,
  testcase,
  ...
  const { recordStatus, recordLists, regex } = recorder;
  ...
  const onGenerateCase = async () => {
    ...
    if (res) {
      ...
      notification.success({
        ...
        description: (
          <span>
            点击<a href={`/#/apiTest/testcase/${res.data.directory_id}/${res.data.id}`}>链接</a>
            可跳转至测试用例
          </span>
        ),
      });
  ...
  const getDownloadUrl = (cert: number) => {
  ...
  return (
    ...
    >
      ...
      <Card>
        <Modal
          title={<span>生成用例 - 已选中{selectedRowKeys.length}条数据</span>}
          open={visible}
          onOk={onGenerateCase}
        ...
        >
          <Form form={form} {...CONFIG.LAYOUT}>
            ...
            >
              <TreeSelect
                placeholder="先选择项目，然后选择你要生成的场景目录"
                treeLine
                treeData={directory}
              />
            </Form.Item>
        ...
        <Row gutter={12}>
          ...
          <Col span={6}>
            ...
            {recordStatus ? (
              <Button onClick={stopRecord} danger style={{ float: 'right', marginRight: 8 }}>
                <StopOutlined />
                停止录制
              </Button>
            ) : (
              ...
              >
                <VideoCameraOutlined />
                {recordLists.length === 0 ? '开始录制' : '重新录制'}
              </Button>
            )}
          </Col>
        </Row>
...
export default connect(({ loading, recorder, project, testcase, global }: any) => ({
  global,
  recorder,
  testcase,
  project,

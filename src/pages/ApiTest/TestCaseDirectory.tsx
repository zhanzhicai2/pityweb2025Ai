...
interface TestCaseDirectoryProps {
  testcase: any;
  gconfig: any;
  project: any;
  user: any;
...
const TestCaseDirectory: React.FC<TestCaseDirectoryProps> = ({
  testcase,
  gconfig,
  project,
  user,
  ...
  const execute = async (record: any, env: number) => {
  ...
  const onExecute = async (env: number) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'request_type',
      width: 110,
      render: (request_type: number) => REQUEST_TYPE[request_type],
    },
    {
      title: '优先级',
      dataIndex: 'priority',
    ...
    },
    {
      ...
      key: 'status',
      width: 110,
      render: (status: number) => <Badge {...CONFIG.CASE_BADGE[status]} />,
    },
    {
      title: '创建人',
  ...
  const onCreateDirectory = async (values: any) => {
    const params = {
      name: values.name,
      project_id,
      parent: currentNode,
  ...
  const onDeleteDirectory = async (key: number) => {
    const res = await dispatch({
  ...
  const getProject = () => {
    ...
    const filter_project = projects.filter((p: any) => p.id === project_id);
  ...
  const onSubmit = async () => {
    ...
    let tempData: any[] = [];
    Object.values(testData).forEach((v: any) => {
...
export default connect(({ testcase, gconfig, project, user, loading }: any) => ({
  loading,
  gconfig,
  user,
  project,
